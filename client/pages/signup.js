...
export default function Signup() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-800 via-blue-600 to-blue-400">
      ...
      <form onSubmit={handleSignup} className="bg-white/80 rounded-lg shadow-lg p-8 flex flex-col gap-4 w-full max-w-md animate-fade-in">
        ...
        <button type="submit" className="bg-gradient-to-r from-blue-600 to-blue-400 text-white px-6 py-2 rounded shadow hover-scale disabled:opacity-50" disabled={loading}>{loading ? "..." : "Sign Up"}</button>
        <a href="/login" className="text-blue-600 text-sm text-center underline">Already have an account? Login</a>
      </form>
    </div>
  );
}
